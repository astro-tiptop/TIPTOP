...
Either make ``sensor_science.Fieldofview`` bigger or play with ``min(source_science.Wavelength-sensor_science.SpectralBandWidth)`` or ``DM.DmPitchs`` or ``sensor_science.PixelScale`` to make this term smaller. If ``sensor_science.FieldOfView = -1``, the automatic choice uses the maximum between twice the AO-corrected size and twice the seeing.
